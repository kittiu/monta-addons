...
class account_invoice_line(osv.osv):
    ...
    def move_line_get(self, cr, uid, invoice_id, context=None):
        ...
        # ================ New code, specific for Cigarette =========================
        if inv.is_cigarette: 
            company_currency = inv.company_id.currency_id.id
            def get_price(cr, uid, inv, company_currency,i_line):
                cur_obj = self.pool.get('res.currency')
                if inv.currency_id.id != company_currency:
                    price = cur_obj.compute(cr, uid, company_currency, inv.currency_id.id, (i_line.product_id.wholesale_standard_price - i_line.product_id.purchase_vat) * i_line.quantity, context={'date': inv.date_invoice})
                else:
                    price = (i_line.product_id.wholesale_standard_price - i_line.product_id.purchase_vat) * i_line.quantity
                return price
                    ...
                    if i_line.product_id and i_line.product_id.valuation == 'real_time':
                        ...
                        if dacc and cacc:
                            res.append({
                                'type':'src',
                                'name': i_line.name[:64],
                                'price_unit': (i_line.product_id.wholesale_standard_price - i_line.product_id.purchase_vat),
